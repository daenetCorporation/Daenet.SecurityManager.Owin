...
namespace Daenet.SecurityManager.Owin
{
    public class ScmAuthorizeAttribute : AuthorizeAttribute
    {
            ...
            if (owinEnv != null)
            {
                ...
                if (actionContext.RequestContext.Principal != null)
                {
                    ...
                    if (actionContext.RequestContext.Principal.Identity.IsAuthenticated)
                    {
                        ...
                        if (res)
                            traceInf("Security Manager user '{0}' authenticated successfully'", secManUserName);
                        else
                            traceInf("Security Manager user '{0}' was not authenticated'", secManUserName);
        ...
        private IDictionary<string, object> getOwinEnvironment(System.Web.Http.Controllers.HttpActionContext actionContext)
        {
            ...
            else if (actionContext.Request.Properties.ContainsKey("MS_OwinEnvironment"))
            {
                return actionContext.Request.Properties["MS_OwinEnvironment"] as IDictionary<string, object>;
            }
            else
                return null;
        }
